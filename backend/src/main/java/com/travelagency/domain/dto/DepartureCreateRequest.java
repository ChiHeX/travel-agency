...
/**
 * 创建团期请求，对齐契约 {@code DepartureCreateRequest}（additionalProperties: false）。
 *
 * <p>只声明契约允许客户端提交的字段。{@code status}、{@code reservedPeople}、
 * {@code confirmedPeople}、{@code version} 都不在契约内，且都由服务端决定：</p>
 * <ul>
 ...
 *   <li>{@code version} 是乐观锁版本号，新建时由服务端从 0 起算。</li>
 * </ul>
 *
 ...
 *
 * <p>修改请求见 {@link DepartureUpdateRequest}：字段相同但额外要求回传读取时的
 * {@code version}，两者刻意分开定义，避免"创建也要传版本"这种语义不清的契约。</p>
 */
